---
title: "Unsettled signals"
subtitle: "Labour market discrimination against Roma in Spain and Travellers in Ireland"
output-dir: "../docs"
author: "Daniel Capistrano, Alvaro Suarez-Vergne, Mat Creighton, <br>Fran McGinnity, Evan Carron-Kee, Hector Cebolla-Boado"
  # - name: Daniel Capistrano
  #   orcid: 0000-0002-8157-5564
  #   affiliations: ESRI
  # - name: Alvaro Suarez-Vergne
  #   orcid: 0000-0003-2787-4560
  #   affiliations: CSIC
  # - name: Mat Creighton
  #   orcid: 0000-0002-1596-6049
  #   affiliations: UCD
  # - name: Fran McGinnity
  #   orcid: 0000-0002-2184-315X
  #   affiliations: ESRI
  # - name: Evan Carron-Kee
  #   orcid: 0009-0001-3105-2554
  #   affiliations: ESRI
  # - name: Hector Cebolla-Boado
  #   orcid: 0000-0002-5804-8715
  #   affiliations: CSIC
  # - name: "."
  #   affiliations: ""
title-slide-attributes:
    data-background-image: img/pres_cover.png
    data-background-size: contain
    data-background-position: bottom
date: Jul 2026
date-format: "MMMM YYYY"
from: markdown+emoji
format: 
  revealjs:
    menu: false
    theme: slides.scss
    incremental: true
    slide-number: false
    auto-stretch: false
    logo: img/logo_esri.png
    transition: fade
    mouse-wheel: true    
    background-transition: fade
    controls: true
    #chalkboard: true
    highlight-style: github
    bibliography: refs.bib
    lightbox: true
editor: 
  markdown: 
    wrap: sentence
embed-resources: true
include-after: |
  <script type="text/javascript">
    Reveal.on('ready', event => {
      if (event.indexh === 0) {
        document.querySelector("div.has-logo > img.slide-logo").style.display = "none";
      }
    });
    Reveal.addEventListener('slidechanged', (event) => {
      if (event.indexh === 0) {
        Reveal.configure({ slideNumber: null });
        document.querySelector("div.has-logo > img.slide-logo").style.display = "none";
      }
      if (event.indexh === 1) { 
        document.querySelector("div.has-logo > img.slide-logo").style.display = null;
      }
    });
  </script>
execute: 
  echo: false
  warning: false
---

# Motivation {background-color="#223451"}

## Labour market disadvantage


::: {.notes}

Common configurations for slides:

{background-image="./img/" background-size="100%"}
{.smaller}
{.scrollable}
{.nonincremental}
{.columns} {.column width="50%"}
{.r-fit-text}

:::



<br>

::: {.columns}


::: {.column width="40%"}
![](img/FRA_Employment.png){fig-align="center"}
:::

::: {.column width="60%"}
- **22%** of Irish Travellers in paid work (79% overall pop.)
- **31%** of Roma in Spain in paid work (71% overall pop.)
- Differences in education/training levels can only partially explain these gaps [@watson_social_2017]

:::

:::

:::{.aside}
Source: European Union Agency for Fundamental Rights. 2025. Rights of Roma and Travellers in 13 European Countries: Perspectives from the Roma Survey 2024. LU: Publications Office.
:::


## Discrimination in the labour marker

<br>

::: {.columns}

::: {.column  width="40%"}
![](img/FRA_Discrimination.png){fig-align="center"}
:::

::: {.column width="60%"}
- **84%** of Irish Travellers  
- **31%** Roma in Spain felt discriminated against when looking for a job  
:::
:::

:::{.aside}
Source: European Union Agency for Fundamental Rights. 2025. Rights of Roma and Travellers in 13 European Countries: Perspectives from the Roma Survey 2024. LU: Publications Office.
:::


# Field experiment {background-color="#223451"}


## 

![](img/es_logowide.png){fig-align="center" width=40%}  

::: {.columns}

::: {.column  width="30%"}
![](img/es_team.png){fig-align="center"}  
:::

::: {.column width="70%"}

- Nine countries [(BE, CH, CZ, DE, ES, HU, IE, NL, GB)]{.small} 
- Childcare, Employment, Housing  
- Current study:
  - Employment  
  - Ireland (1,625) and Spain (1,793) 
  - Cook, Electrician, Forkift driver, Clerk, Receptionist, Store assistant, Sales rep., Comms. manager, Software developer
:::
:::

## Callback rates

![](../output/plot_callback.png){fig-align="center" width=80%}


## Discrimination ratios

![](../output/plot_discratio.png){fig-align="center" width=80%}

## Names as ethnic markers

- Correspondence studies rely on names as signals for characteristics such as gender, ethnicity, religion, race and class [@elder_signaling_2023; @crabtree_validated_2023; @ghekiere_perception_2025] 

- Differences in perception of the signals transmitted by these names [@martiniello_signaling_2022; @crabtree_racially_2022; @bauer_beware_2026]

# Names survey{background-color="#223451"}

## Names survey 

::: {.nonincremental}
- Websurvey conducted between Autumn 2023 and early 2024   
- First names selected among most common on Live Births Registration   
- Surnames selected in consultation with community organisations   
:::
- **Ireland**: 900 participants assessed 10 names each from a list of 130 names  
- **Spain**: 512 participants assessed 10 names each from a list 140 names

## Identifying names as ethnic majority

![](../output/plot_majority.png){fig-align="center" width=70%}

## Identifying names as ethnic minority

![](../output/plot_minority.png){fig-align="center" width=70%}

## SES of names

![](../output/plot_ses.png){fig-align="center" width=70%}

## Research question  

<br>

> Is there a relationship between the average signal strength of the name and callback rates?  

<br>

:::{.callout-note title="Signal strength of the name" .fragment .small}
Majority: proportion indicating country of origin   
Muslim: mean value of religiosity scale   
Black: mean value skin tone scale   
Travellers/Roma: proportion indicating expected ethnicity   
:::


# Main results {background-color="#223451"}


## Correlation

![](../output/plot_cor_nm_signal_std.png){fig-align="center" width=90%}


## Model specification

- Outcome variable: Positive response  
- Main variable: signal strength  
- Linear probability model  
- Pooled sample with country fixed effects  
- Cluster robust standard errors  

## Model main estimates

<br>

```{r}
library(tidyverse) # for data manipulation
library(haven) # to import/export from/to SPSS/STATA formats
library(gt) # formatted tables
library(gtsummary)
library(marginaleffects)

source("../src/0_utils.R")
source("../src/1_import.R")

vars_control <- c(
  "applicant_female", "parent", "app_citizenship", "past_unemployment",
  "fulltime_job", "childcare_success", "housing_success", "occupation"
)

fit_default_model <- function(covars, outcome = "lm_positive",  interaction = FALSE){
  rhs_vars <- c(covars, vars_control)
  get_disc_model(
    df_exp, outcome = outcome, covars = rhs_vars, 
    se = "cluster", cluster_var = "name_applicant", interaction = interaction
  )
}

get_output_table <- function(m1, m2, vars2){
  tbl_merge(list(
      tbl_regression(m1, include = "name_mingroup", conf.int = FALSE, intercept = TRUE) |> bold_p(),
      tbl_regression(m2, include = c("name_mingroup", vars2), conf.int = FALSE, intercept = TRUE) |> bold_p()),
    tab_spanner = c("Model 1", "Model 2")
  ) |> 
  as_gt() |> 
  gt::text_case_match("nm_signal_std" ~ "Signal strength", "name_mingroup" ~ "Ethnic group")
}

m_naive  <- fit_default_model(covars = c("name_mingroup", "country"))
m_signal  <- fit_default_model(covars = c("name_mingroup", "nm_signal_std", "country"))

get_output_table(m_naive, m_signal, c("name_mingroup", "nm_signal_std"))

```

:::{.aside}
Models include *country* fixed effects and the following controls: *gender*, *occupation*, *parental status*, *citizenship*, *past unemployment*, *full time position*, *childcare success*, *housing success*.
:::

## Interaction between group and signal strength

![](../output/plot_interact.png){fig-align="center" width=90%}

:::{.aside}
Model includes country fixed effects and the following controls: gender, occupation, parental status, citizenship, past unemployment, full time position, childcare success, housing success.
:::

## Final considerations

- **Limitations**:  Small number of names, sample hetereogeneity   
- **Sensitivity analysis**: [Invitation](#predicted-values---invitation) and [rejection](#predicted-values---rejection) as outcome, logistic regression  
- **Next steps**: 
  - Name recognition for gatekeepers' profile (cob / education)   
  - Name recognition at the sub-national level   
  - Relationship with [social status](#predicted-values---ses)


## Thank you {.hide-logo .inverted }
<br>

Daniel Capistrano ([daniel.capistrano@esri.ie](daniel.capistrano@esri.ie))  
Alvaro Suarez, Mat Creighton, Fran McGinnity, Evan Carron-Kee, Hector Cebolla

<br>

::: {.columns}
::: {.column width="20%" text-align="center"}
![](./img/news_qrcode.png){fig-align="center"}

:::{style="font-size: 40%;"}
Sign up to our Newsletter
:::

:::
::: {.column width="80%" .small}

![](./img/icon_website.svg){width="4%" style="vertical-align: middle;"}  [ESRI.ie](https://www.esri.ie)  
![](./img/icon_linkedin.svg){width="4%" style="vertical-align: middle;"} [Economic and Social Research Institute](https://ie.linkedin.com/company/economic-and-social-research-institute-esri-)  
![](./img/icon_youtube.svg){width="4%" style="vertical-align: middle;"} [\@ESRIDublin](https://www.youtube.com/@ESRIDublin)  
![](./img/icon_bluesky.svg){width="4%" style="vertical-align: middle;"} [\@ESRI.ie](https://bsky.app/profile/esri.ie)  

:::

:::

## References
:::{#refs}
:::

# Additional analyses {background-color="#223451"}


## Predicted values - Logistic

![](../output/plot_logit_interact.png){fig-align="center" width=90%}

## Predicted values - Invitation

![](../output/plot_interact_inv.png){fig-align="center" width=90%}

:::{.aside}
[back](#final-considerations)
:::

## Predicted values - Rejection

![](../output/plot_interact_rej.png){fig-align="center" width=90%}

:::{.aside}
[back](#final-considerations)
:::

## Correlation - SES

![](../output/plot_cor_ladder_adj_mean.png){fig-align="center" width=80%}

:::{.aside}
[back](#final-considerations)
:::

## Predicted values - SES

![](../output/plot_interact_ses.png){fig-align="center" width=90%}

:::{.aside}
[back](#final-considerations)
:::

## Selection of names for the field experiment 

- Majority: Country origin  
- Muslim: Religiosity  
- Black: Skin tone  
- Roma/Travellers: Direct question ethnicity     
